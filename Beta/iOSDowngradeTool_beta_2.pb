...
Dim filesSizes.i(6) : filesSizes(0) = 73728 : filesSizes(1) = 77824 : filesSizes(2) = 52064 : filesSizes(3) = 335872 : filesSizes(4) = 352256 : filesSizes(5) = 447568
Dim firmSizes.i(6) : firmSizes(0) = 1656868596 : firmSizes(1) = 1658106551 : firmSizes(2) = 967088093 : firmSizes(3) = 835654312 : firmSizes(4) = 852289339 : firmSizes(5) = 850330223
...
If OpenWindow(0, #PB_Ignore, #PB_Ignore, 250, 135, "iOS Downgrade Tool")
  ...
  SetGadgetState(2, 0)
  AddGadgetItem(2, 1, "iPhone5,2")
  AddGadgetItem(2, 2, "iPhone5,1")
  AddGadgetItem(2, 3, "iPhone4,1")
  AddGadgetItem(2, 4, "iPad2,1")
  AddGadgetItem(2, 5, "iPad2,2")
  AddGadgetItem(2, 6, "iPad2,3")

  CheckBoxGadget(3, 188, 2, 60, 25, "No SSH")
  CheckBoxGadget(4, 6, 57, 160, 20, "Не скачивать прошивку")

  TextGadget(5, 203, 125, 50, 15, "v1.3 beta 2")
    ...
    If Event = #PB_Event_Gadget
      Select EventGadget()
        Case 1
          ...
          EndIf
        Case 2
          If CountGadgetItems(2) = 7 : RemoveGadgetItem(2, 0) : EndIf

          If Not GetGadgetState(2) Or GetGadgetState(2) = 1 : MessageRequester("Attention", "iPhone5,1/5,2 можно откатить только до iOS 8.4.1") : EndIf
        Case 3
        ...
        Case 7
          ...
          If (Device And CountGadgetItems(2) = 6) And (NoSSH Or (ipAddress And ipAddress <> "IP-Адрес"))
            If Not FileSize(wget) = -1
              If Not FileSize(Device) = -2
                CreateDirectory(Device)
              EndIf
              If FileSize(Device + "\pwnediBSS") <> filesSizes(0) And FileSize(Device + "\pwnediBSS") <> filesSizes(1)
